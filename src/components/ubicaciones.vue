      ...
      methods: {
        ...
        async borrarActivos() {
          if(this.editedIndex != -1) {
            const activos = await axios.get(`https://localhost:4000/ubicacion/${this.editedItem.id}/activos`, {
              headers: {
                Authorization: `Bearer ${this.token}`
              }
            })
            for (let i = 0; i < activos.data.length; i++) {
              ...
              axios.put(`https://localhost:4000/activo/${activo.id}/borrarUbicacion`, {
                headers: {
                  Authorization: `Bearer ${this.token}`
                }
              })
